...
export default function SubmissionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!campaignId) { toast.error("Pilih campaign terlebih dahulu"); return; }

    setSubmitting(true);
    try {
      await submitVideo({ campaignId, platform: platform || undefined, platformLink: platformLink || undefined });
      toast.success("Video berhasil disubmit! Menunggu review.");
      setShowForm(false);
      form.reset();
      fetchData();
    } catch (err: any) {
      toast.error(err.message || "Gagal submit video");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8888aa]">Pantau status video yang sudah kamu submit.</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
          <h3 className="mb-4 text-lg font-semibold text-white">Submit Video Baru</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-[#e8e8f0]">Campaign</label>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-[#e8e8f0]">Platform</label>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-[#e8e8f0]">Link Video</label>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center text-[#8888aa]">
          <p className="text-lg">Belum ada submission.</p>
        </div>
      ) : (
              ...
              <div key={sub.id} className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-white">{sub.campaign?.title || "Unknown Campaign"}</h4>
                      <div className="mt-1 flex items-center gap-3 text-sm text-[#8888aa]">
                        ...
                        {sub.platformLink && (
                          <a href={sub.platformLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[#6c63ff] hover:underline">
                            <ExternalLink className="h-3 w-3" /> Lihat Video
                          </a>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-center">
                      ...
                      <div className="text-xs text-[#8888aa]">Views</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-[#10b981]">Rp {estimatedPayout.toLocaleString()}</div>
                      <div className="text-xs text-[#8888aa]">Estimasi</div>
                    </div>
