  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#8888aa]">Campaign yang sudah kamu ikuti.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <h3 className="mt-4 text-lg font-semibold text-white">Belum ada campaign</h3>
          <p className="text-[#8888aa]">Explore campaign yang tersedia dan mulai dapatkan cuan!</p>
          <a href="/creator/explore" className="mt-4 rounded-xl bg-gradient-to-r from-[#6c63ff] to-[#3b82f6] px-6 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity">
            Explore Campaigns
          </a>
        </div>
      ) : (
              ...
              <div key={c.id} className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-[#8888aa]">Joined {timeAgo(new Date(c.joinedAt))} &middot; CPM: Rp {c.cpmRate.toLocaleString()}/1K</p>
                  </div>
                  <div className="flex gap-6">
                    <div className="text-center">
                      <div className="text-lg font-bold text-white">{c.submissionsCount}</div>
                      <div className="text-xs text-[#8888aa]">Submissions</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-white">{c.totalViews.toLocaleString()}</div>
                      <div className="text-xs text-[#8888aa]">Total Views</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-[#10b981]">{formatCurrency(c.earnings)}</div>
                      <div className="text-xs text-[#8888aa]">Earnings</div>
                    </div>
                  </div>
                </div>
              </div>
