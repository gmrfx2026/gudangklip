  ...
  const handleInvite = () => {
    if (inviteLink) {
      navigator.clipboard.writeText(inviteLink).then(() => {
        ...
        toast.info(`Bagikan link: ${inviteLink}`);
      });
    } else {
      toast.info("Referral code sedang dibuat...");
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8888aa]">Kelola semua kreator dalam agency kamu.</p>
        </div>
        <button onClick={handleInvite} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#6c63ff] to-[#3b82f6] px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90">
          {copied ? <><Check className="h-4 w-4" /> Tersalin</> : <><UserPlus className="h-4 w-4" /> Invite Creator</>}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center text-[#8888aa]">
          <p className="text-lg">Belum ada member.</p>
        </div>
      ) : (
