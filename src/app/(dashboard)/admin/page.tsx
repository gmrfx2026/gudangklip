...
export default function AdminOverview() {
  ...
  const handleApprove = async (campaignId: string) => {
    try {
      ...
      toast.error(err.message || "Gagal menyetujui campaign");
    }
  };
  ...
  const handleReject = async (campaignId: string) => {
    try {
      ...
      toast.error(err.message || "Gagal menolak campaign");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#8888aa]">Overview platform GudangKlip.</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Total Users", value: data.totalUsers.toLocaleString(), icon: <Users className="h-5 w-5" /> },
          { label: "Active Campaigns", value: String(data.activeCampaigns), icon: <Megaphone className="h-5 w-5" /> },
          { label: "Total Payout", value: formatCurrency(data.totalPayout), icon: <BarChart3 className="h-5 w-5" /> },
          { label: "Pending Reviews", value: String(data.pendingSubmissions), icon: <Shield className="h-5 w-5" /> },
        ].map((s) => (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
          ...
          {data.recentUsers.length === 0 ? (
            <p className="py-8 text-center text-sm text-[#8888aa]">Belum ada user.</p>
          ) : (
        ...
        <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
          <h3 className="mb-4 text-lg font-semibold text-white">Pending Campaign Approvals</h3>
          {data.pendingCampaigns.length === 0 ? (
            <p className="py-8 text-center text-sm text-[#8888aa]">Tidak ada campaign yang menunggu approval.</p>
          ) : (
            <div className="space-y-3">
              {data.pendingCampaigns.map((c) => (
                <div key={c.id} className="flex items-center justify-between rounded-xl bg-[#0d0d22] p-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleApprove(c.id)} className="rounded-lg bg-[#10b981]/10 px-3 py-1.5 text-xs font-medium text-[#10b981] hover:bg-[#10b981]/20">Approve</button>
                    <button onClick={() => handleReject(c.id)} className="rounded-lg bg-[#ef4444]/10 px-3 py-1.5 text-xs font-medium text-[#ef4444] hover:bg-[#ef4444]/20">Reject</button>
                  </div>
                </div>
              ))}
            </div>
