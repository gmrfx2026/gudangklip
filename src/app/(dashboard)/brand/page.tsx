    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8888aa]">Kelola campaign dan pantau performa brand kamu.</p>
        </div>
        <a
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { icon: <Megaphone className="h-5 w-5" />, label: "Active Campaigns", value: String(data.activeCount) },
          { icon: <TrendingUp className="h-5 w-5" />, label: "Total Views", value: formatCompactNumber(data.totalViews) },
          { icon: <DollarSign className="h-5 w-5" />, label: "Budget Digunakan", value: formatCurrency(data.usedBudget) },
          { icon: <Users className="h-5 w-5" />, label: "Creators Aktif", value: String(data.uniqueCreators) },
        ].map((stat) => (
          <div key={stat.label} className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-5">
            ...
            <div className="mt-1 text-xs text-[#8888aa]">
              {budgetPercent}% dari total budget
            </div>
          </div>
        ))}
      </div>
