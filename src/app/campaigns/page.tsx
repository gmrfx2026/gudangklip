      ...
      <nav className="fixed top-0 z-50 w-full border-b border-[#2a2a50]/50 bg-[#0a0a1a]/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-4">
            <Link href="/login" className="text-sm text-[#b8b8d0] hover:text-white">Login</Link>
            <Link href="/register" className="rounded-full bg-gradient-to-r from-[#6c63ff] to-[#3b82f6] px-5 py-2 text-sm font-semibold text-white hover:opacity-90">Daftar</Link>
          </div>
        </div>
      </nav>
      ...
      <main className="pt-24 pb-20">
        <div className="mx-auto max-w-7xl px-4">
          <div className="mb-8 text-center">
            <h1 className="mb-2 text-3xl font-bold text-white">Explore Campaigns</h1>
          ...
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2 overflow-x-auto">
              <button onClick={() => setCategory("")} className={`rounded-xl px-4 py-2 text-xs font-medium whitespace-nowrap ${!category ? "bg-[#6c63ff] text-white" : "border border-[#2a2a50] text-[#b8b8d0]"}`}>Semua</button>
              {CATEGORIES.map((cat) => (
                ...
                return (
                  <div key={c.id} className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 overflow-hidden hover:border-[#6c63ff]/50 transition-all">
                    ...
                    <div className="p-5">
                      ...
                      <p className="mb-3 text-xs text-[#8888aa]">{c.brand.name || "Unknown Brand"}</p>
                      <div className="mb-3 flex items-center justify-between text-sm">
                        <div className="flex items-center gap-1 text-[#10b981]"><DollarSign className="h-4 w-4" /><span className="font-semibold">Rp {c.cpmRate.toLocaleString()} /1K</span></div>
                        <div className="flex items-center gap-1 text-[#8888aa]"><Users className="h-4 w-4" /><span>{c._count.participants}</span></div>
                      </div>
                      <div className="mb-1 flex justify-between text-xs">
                        <span className="text-[#8888aa]">Budget</span><span className="text-white">{formatCurrency(c.totalBudget)}</span>
                      </div>
