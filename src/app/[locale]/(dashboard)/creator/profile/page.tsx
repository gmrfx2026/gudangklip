...
export default function ProfilePage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t("CreatorProfile.toastUploadFailed"));

      await updateProfileImage(data.url);
      await updateSession();
      toast.success(t("CreatorProfile.toastImageUpdated"));
      fetchData();
    } catch (err: any) {
      toast.error(err.message || t("CreatorProfile.toastUploadFailed"));
    } finally {
      setUploading(false);
  ...
  const handleConnect = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!username) { toast.error(t("CreatorProfile.usernameRequired")); return; }

    setConnecting(true);
    try {
      await connectSocialAccount({ platform, username });
      toast.success(t("CreatorProfile.toastAccountConnected"));
      setShowConnect(false);
      form.reset();
      fetchData();
    } catch (err: any) {
      toast.error(err.message || t("CreatorProfile.toastAccountConnectFailed"));
    } finally {
      setConnecting(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-white">{t("CreatorProfile.socialAccounts")}</h3>
          <button
            onClick={() => setShowConnect(!showConnect)}
            className="flex items-center gap-1 text-sm font-medium text-[#6c63ff] hover:underline"
          >
            <Plus className="h-4 w-4" /> {t("CreatorProfile.connectAccount")}
          </button>
        </div>
        ...
        {showConnect && (
          <div className="mb-6 rounded-xl border border-[#2a2a50] bg-[#0d0d22] p-4">
            <form onSubmit={handleConnect} className="space-y-3">
              <div>
                <label className="mb-1 block text-sm font-medium text-[#e8e8f0]">{t("CreatorProfile.platform")}</label>
                ...
                <div className="flex items-center gap-2">
                  {acc.verified && (
                    <span className="flex items-center gap-1 rounded-full bg-[#10b981]/10 px-2.5 py-1 text-xs font-medium text-[#10b981]">
                      <Check className="h-3 w-3" /> {t("CreatorProfile.verified")}
                    </span>
                  )}
                </div>
              </div>
