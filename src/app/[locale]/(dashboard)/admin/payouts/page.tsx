...
export default function AdminPayouts() {
  ...
  const handleProcess = async (payoutId: string, status: "PROCESSING" | "COMPLETED" | "FAILED") => {
    ...
    try {
      ...
      toast.success(t("AdminPayouts.toastUpdateSuccess"));
      fetchPayouts();
    } catch (err: any) {
      toast.error(err.message || t("AdminPayouts.toastUpdateFailed"));
    } finally {
      setProcessing(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto">
        <button onClick={() => setFilter("")} className={`rounded-xl px-4 py-2 text-xs font-medium whitespace-nowrap ${!filter ? "bg-[#6c63ff] text-white" : "border border-[#2a2a50] text-[#b8b8d0]"}`}>
          {t("AdminPayouts.filterAll")} ({payouts.length})
        </button>
            ...
            <table className="w-full">
              <thead className="border-b border-[#2a2a50]">
                <tr className="text-left text-xs font-medium text-[#8888aa]">
                  ...
                  <th className="px-6 py-4">{t("AdminPayouts.colAccountInfo")}</th>
                  <th className="px-6 py-4">{t("AdminPayouts.colStatus")}</th>
                  <th className="px-6 py-4">{t("AdminPayouts.colAction")}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((p) => {
                        ...
                        {p.status === "PROCESSING" && (
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="rounded-lg bg-[#ef4444]/10 px-2.5 py-1 text-xs font-medium text-[#ef4444] hover:bg-[#ef4444]/20 disabled:opacity-50"
                            >
                              {t("AdminPayouts.actionFail")}
                            </button>
                          </div>
                        )}
                        {p.status === "COMPLETED" && (
                          <span className="text-xs text-[#10b981]">{t("AdminPayouts.statusDone")}</span>
                        )}
                        {p.status === "FAILED" && (
                          <span className="text-xs text-[#ef4444]">{t("AdminPayouts.statusRefunded")}</span>
                        )}
                      </td>
                    </tr>
                  );
