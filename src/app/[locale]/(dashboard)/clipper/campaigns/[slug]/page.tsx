...
const statusConfig: Record<string, { icon: React.ReactNode; color: string; bg: string; label: string }> = {
  PENDING: { icon: <Clock className="h-3.5 w-3.5" />, color: "text-[#f59e0b]", bg: "bg-[#f59e0b]/10", label: "Pending" },
  APPROVED: { icon: <CheckCircle className="h-3.5 w-3.5" />, color: "text-[#10b981]", bg: "bg-[#10b981]/10", label: "Approved" },
  REJECTED: { icon: <XCircle className="h-3.5 w-3.5" />, color: "text-[#ef4444]", bg: "bg-[#ef4444]/10", label: "Rejected" },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        <Link href="/clipper/campaigns" className="text-[#a0a0c0] hover:text-white">Campaigns</Link>
        ...
        <div className="relative p-6">
          ...
          <div className="mt-4 flex items-start gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-[#6c63ff]/20 to-[#3b82f6]/20">
              <span className="text-lg font-bold text-[#6c63ff]">{campaign.brand?.name?.charAt(0) || "B"}</span>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-[#10b981]/10 px-1.5 py-0.5 text-[10px] font-medium text-[#10b981]">{campaign.status.toLowerCase()}</span>
              </div>
              <h1 className="mt-1 text-2xl font-bold text-white">{campaign.title}</h1>
              <div className="mt-1 text-lg font-bold text-[#10b981]">
                Rp{campaign.cpmRate.toLocaleString()} <span className="text-xs font-normal text-[#a0a0c0]">/ 1K views</span>
              </div>
            </div>
          </div>
          <div className="mt-4 flex items-center gap-3">
            {PLATFORMS.slice(0, 3).map((p) => (
              <span key={p.value} className="text-xs text-[#a0a0c0]">{p.label}</span>
            ))}
          ...
          <div className="mt-4 flex gap-2">
            {!isJoined ? (
              ...
              >
                {hasMySubmission ? "Sudah Dikirim" : "Submit Video"}
              </button>
            )}
      ...
      <div className="flex gap-1 rounded-xl border border-[#2a2a50] bg-[#0d0d22] p-1">
        <button onClick={() => setActiveTab("detail")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${activeTab === "detail" ? "bg-[#6c63ff] text-white" : "text-[#a0a0c0] hover:text-white"}`}>Detail</button>
        <button onClick={() => setActiveTab("videos")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${activeTab === "videos" ? "bg-[#6c63ff] text-white" : "text-[#a0a0c0] hover:text-white"}`}>Video Kamu</button>
      </div>
      ...
      {activeTab === "detail" ? (
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
            <h2 className="mb-3 text-lg font-semibold text-white">Tentang Campaign</h2>
            <p className={`text-sm text-[#a0a0c0] ${!showFullDesc && "line-clamp-3"}`}>
              {campaign.description || "Bergabunglah dalam campaign ini dan dapatkan penghasilan dari setiap views video yang kamu buat."}
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Link href={`/clipper/campaigns/${slug}/brief`} className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-5 hover:border-[#6c63ff]/50 transition-all">
              <h3 className="mb-1 text-base font-semibold text-white">Brief</h3>
              <p className="text-xs text-[#a0a0c0]">Penjelasan brand, narasi, hashtag, dan aturan campaign</p>
            </Link>
            <Link href={`/clipper/campaigns/${slug}/materi`} className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-5 hover:border-[#6c63ff]/50 transition-all">
              <h3 className="mb-1 text-base font-semibold text-white">Materi Clipping</h3>
              <p className="text-xs text-[#a0a0c0]">Logo, footage, dan material yang harus dipakai</p>
            </Link>
          </div>
          ...
          <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
            <div className="grid gap-4 sm:grid-cols-4">
              <div className="text-center">
                <div className="mb-1 text-sm text-[#a0a0c0]">Budget Tersisa</div>
                <div className="text-xl font-bold text-white">{budgetPercent}%</div>
              </div>
              <div className="text-center">
                <div className="mb-1 text-sm text-[#a0a0c0]">Min View to Claim</div>
                <div className="text-xl font-bold text-white">{formatCompactNumber(campaign.minViewsToClaim || 0)} Views</div>
              </div>
              <div className="text-center">
                <div className="mb-1 text-sm text-[#a0a0c0]">Max Views per Video</div>
                <div className="text-xl font-bold text-white">{campaign.maxViewsPerVideo ? formatCompactNumber(campaign.maxViewsPerVideo) : "∞"}</div>
              </div>
              <div className="text-center">
                <div className="mb-1 text-sm text-[#a0a0c0]">CPM / 1K Views</div>
          ...
          <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">Top 10 Clip</h3>
              <button onClick={fetchSubmissions} className="flex items-center gap-2 rounded-lg border border-[#2a2a50] px-3 py-1.5 text-xs text-[#a0a0c0] hover:bg-[#1e1e3f] transition-colors">
                <RefreshCw className="h-3.5 w-3.5" /> Refresh Views
              </button>
            </div>
            {top10.length === 0 ? (
              <p className="py-8 text-center text-sm text-[#6b7280]">Belum ada clip yang masuk untuk campaign ini.</p>
            ) : (
                  ...
                  <div key={entry.id} className="flex items-center gap-4 rounded-xl border border-[#1e1e3f] bg-[#0d0d22]/50 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#a0a0c0]">
                        {PLATFORMS.find((p) => p.value === entry.platform)?.label || "N/A"} &middot; {formatCompactNumber(entry.views)} views
                      </p>
                    </div>
                    {entry.videoUrl && (
        ...
        <div className="rounded-2xl border border-[#2a2a50] bg-[#111128]/50 p-6">
          {mySubmissions.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-[#a0a0c0]">Belum ada video untuk campaign ini.</p>
                  ...
                  <div key={sub.id} className="flex flex-col gap-4 rounded-xl border border-[#1e1e3f] bg-[#0d0d22]/50 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-start gap-4">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-[#1e1e3f] to-[#0f0f23] text-sm font-bold text-[#a0a0c0]">
                        {PLATFORMS.find((p) => p.value === sub.platform)?.label?.charAt(0) || "?"}
                      </div>
                      <div>
                        ...
                        <div className="mt-1 flex items-center gap-3 text-xs text-[#a0a0c0]">
                          <span>{PLATFORMS.find((p) => p.value === sub.platform)?.label || "N/A"}</span>
